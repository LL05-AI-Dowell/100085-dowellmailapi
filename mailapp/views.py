...
@method_decorator(csrf_exempt, name='dispatch')
class candidate_removal(APIView):
    def post(self, request):
        email_content = request.data.get('email_content')
        toname = request.data.get('toname')
        toemail = request.data.get('toemail')
        ...
        if email_validation['status'] == "valid":
            ...
            to = [{"email": toemail, "name": toname}]
            bcc = [{"email": "suhair@dowellresearch.in" , "name":"Suhair"},{"email": "rainamary@dowellresearch.com" , "name":"Raina Mary"},{"email":"zr5530213@gmail.com","name":"Zoya"}]
